...
func gPartOneU4(ar []uint32, pv uint32, ch dedego.Chan[int]) {
	ch.Send(partOneU4(ar, pv))
}

func partConU4(slc []uint32, ch dedego.Chan[int]) int {

	pv := pivotU4(slc, nsConc)
	mid := len(slc) >> 1
	...
	func() {
		...
		go func() {
			dedego.SpawnPost(DedegoRoutineIndex)
			{
				...
				gPartOneU4(slc[l:h:h], pv, ch)
			}
		}()
	}()

	r := partTwoU4(slc, l, h, pv)
