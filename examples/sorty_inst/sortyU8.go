...
func gPartOneU8(ar []uint64, pv uint64, ch dedego.Chan[int]) {
	ch.Send(partOneU8(ar, pv))
}

func partConU8(slc []uint64, ch dedego.Chan[int]) int {

	pv := pivotU8(slc, nsConc)
	mid := len(slc) >> 1
	...
	func() {
		...
		go func() {
			dedego.SpawnPost(DedegoRoutineIndex)
			{
				...
				gPartOneU8(slc[l:h:h], pv, ch)
			}
		}()
	}()

	r := partTwoU8(slc, l, h, pv)
